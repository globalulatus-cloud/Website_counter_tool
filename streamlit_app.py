    ...
    if st.button("Analyze Content", key="analyze_single"):
        ...
        if not urls:
            ...
            with st.spinner("Analyzing content..."):
                ...
                for url in urls:
                    try:
                        text, title, _html = run_async(fetch_website_text(url))
                ...
                with col1:
                    st.markdown(
                        f'<div class="metric-card"><div class="metric-label">Total Count</div><div class="metric-value">{total_count:,}</div></div>',
                        unsafe_allow_html=True,
                    )
                with col2:
                    st.markdown(
                        f'<div class="metric-card"><div class="metric-label">Pages</div><div class="metric-value">{len(results)}</div></div>',
                        unsafe_allow_html=True,
                    )
                with col3:
                    primary_group = results[0]["Group"] if results else "-"
                    st.markdown(
                        f'<div class="metric-card"><div class="metric-label">Primary Mode</div><div class="metric-value">{primary_group}</div></div>',
                        unsafe_allow_html=True,
                    )
                ...
                excel_data = to_excel_bytes(df, sheet_name="Single_Page_Report")

                st.download_button(
                    label="Download Excel",
                    data=excel_data,
                    file_name="ulatus_single_page_report.xlsx",
    ...
    if st.button("Start Crawl", key="analyze_crawl"):
        if not root_url.strip():
            ...
            with st.spinner("Crawling website..."):
                try:
                    crawl_results = run_async(crawl_site(root_url.strip(), max_pages=int(max_pages)))

                    if not crawl_results:
                        st.warning("No pages found or analysis failed.")
                    else:
                        total_count = sum(r["stats"]["count"] for r in crawl_results)
                        primary_group = "CJK" if any(
                            r["stats"]["language_group"] == "CJK" for r in crawl_results
                        ) else "Latin"
                        ...
                        with col2:
                            st.markdown(
                                f'<div class="metric-card"><div class="metric-label">Pages Crawled</div><div class="metric-value">{len(crawl_results)}</div></div>',
                                unsafe_allow_html=True,
                            )
                        with col3:
                            st.markdown(
                        ...
                        excel_crawl = to_excel_bytes(df_crawl, sheet_name="Crawl_Report")

                        st.download_button(
                            label="Download Excel",
                            data=excel_crawl,
                            file_name="ulatus_crawl_report.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key="download_crawl",
                        )
